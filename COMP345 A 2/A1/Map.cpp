...
Map::Map(vector<vector<string> > parsedFile)
{
	...
	for (int i = 0; i < parsedFile.size(); i++)
	{
		...
		Continent c(n, p, new vector<Territory>(list_t));
		m.push_back(c);
		c.~Continent();
	}
	continents = new vector<Continent>(m);
...
Map::Map(const Map &map)
{
	...
	this->continents = new vector<Continent>(*(map.continents));
};
...
{
	out << "Map name is " << *map.mapName << endl
		<< "It has " << *map.nbContinents << " continents" << endl;
	return out;
}
// destructor
Map::~Map()
{
	...
	mapName = nullptr;
	vector<Continent> v;
	continents = new vector<Continent>(v);
	delete continents;
	continents = nullptr;
}
...
Map::Territory::Territory(string *name){
	cout << "Territory being targetted or defended by the player.\n";
}
...
{
	out << "Continent name is " << *continent.Cname << endl
		<< "The player the owns the continent scores " << *continent.nbPts << " points" << endl
		<< "and its territories are : insert territories iterator here" << endl;
	return out;
}
